...
`public/resume.pdf` 파일을 추가해서 같이 커밋하면 앱이 시작될 때 자동으로 해당 PDF를 엽니다.
...
파일명은 현재 앱 코드 기준으로 `resume.pdf`를 찾습니다.

## 사용 방법

1. 저장소에 `public/resume.pdf`가 있으면 자동으로 뷰어에 표시
...
- `public/resume.pdf`처럼 정적으로 포함한 파일은 배포 후 실제 URL로 공유할 수 있습니다.
